...
class SpecFileReaderSpec extends FlatSpecLike with Matchers {
  // This was originally testing specifics of the PRA config setup, so this setup is substantially
  // more complicated than necessary.  But, it doesn't hurt anything to have some really nested
  // stuff, so I'll keep it.
